    ...
    while(/* block is of type IND_INTERNAL */StaticBuffer::getStaticBlockType(block) == IND_INTERNAL) {  //use StaticBuffer::getStaticBlockType()
        ...
        if (/* op is one of NE, LT, LE */op == NE||op == LT||op == LE) {
            ...
            */
            ...
            if (/* such an entry is found*/found) {
...
int BPlusTree::bPlusCreate(int relId, char attrName[ATTR_SIZE]) {
    ...
    if (/* an index already exists for the attribute (check rootBlock field) */attrcatbuf.rootBlock!=-1) {
        return SUCCESS;
    }
...
int BPlusTree::bPlusInsert(int relId, char attrName[ATTR_SIZE], Attribute attrVal, RecId recId) {
    ...
    int blockNum = /* rootBlock of B+ Tree (from attrCatEntry) */attrcatbuf.rootBlock;
    ...
    int leafBlkNum = /* findLeafToInsert(root block num, attrVal, attribute type) */findLeafToInsert(blockNum,attrVal,attrcatbuf.attrType);
    ...
    Index leaf;
    leaf.attrVal = attrVal;
    leaf.block = recId.block;
    leaf.slot = recId.slot;
    if (/*insertIntoLeaf() returns E_DISKFULL */insertIntoLeaf(relId,attrName,leafBlkNum,leaf) == E_DISKFULL) {
...
int BPlusTree::findLeafToInsert(int rootBlock, Attribute attrVal, int attrType) {
    ...
    while (/*block is not of type IND_LEAF */StaticBuffer::getStaticBlockType(blockNum)!=IND_LEAF) {  // use StaticBuffer::getStaticBlockType()
        ...
        int index = 0;
        for(index = 0;index<head.numEntries;index++)
        {
            InternalEntry inter;
            internal.getEntry(&inter,index);
            if(compareAttrs(attrVal,inter.attrVal,attrType)<=0)break;
        }
        if (/*no such entry is found*/index == head.numAttrs) {
            // set blockNum = rChild of (nEntries-1)'th entry of the block
            // (i.e. rightmost child of the block)
            InternalEntry inter;
            internal.getEntry(&inter,head.numEntries-1);
            blockNum = inter.rChild;

        } else {
            // set blockNum = lChild of the entry that was found
            InternalEntry inter;
            internal.getEntry(&inter,index);
            blockNum = inter.lChild;
        }
    }
...
int BPlusTree::insertIntoLeaf(int relId, char attrName[ATTR_SIZE], int blockNum, Index indexEntry) {
    ...
    int ind = 0;
    for(ind = 0;ind<blockHeader.numEntries;ind++)
    {
        Index entry;
        leaf.getEntry(&entry,ind);
        if(compareAttrs(entry.attrVal,indexEntry.attrVal,attrcatbuf.attrType)<=0)indices[ind] = entry;//less than then
        else
        {
            indices[ind] = indexEntry;
            break;
        }
    }
    for(int i = ind;i<blockHeader.numEntries;i++)
    {
        Index entry;
    ...
    */
    ...
    if (/* the current leaf block was not the root */blockHeader.pblock!=-1) {  // check pblock in header
        ...
        middle.rChild = newRightBlk;
        int ret = insertIntoInternal(relId,attrName,blockHeader.pblock,middle);
        if(ret != SUCCESS)return ret;
...
int BPlusTree::splitLeaf(int leafBlockNum, Index indices[]) {
    ...
    for(int i =0 ;i<=MIDDLE_INDEX_LEAF;i++)leftBlk.setEntry(&indices[i],i);
    for(int i = 0;i<=MIDDLE_INDEX_LEAF;i++)rightBlk.setEntry(&indices[MIDDLE_INDEX_LEAF+i+1],i);
...
int BPlusTree::insertIntoInternal(int relId, char attrName[ATTR_SIZE], int intBlockNum, InternalEntry intEntry) {
   ...
   {
    InternalEntry entry;
    intBlk.getEntry(&entry,ind);
    if(compareAttrs(entry.attrVal,intEntry.attrVal,attrcatbuf.attrType<=0))internalEntries[ind] = entry;
    else
    {
        if(ind>0)internalEntries[ind-1].rChild = intEntry.lChild;
        if(ind<blockHeader.numEntries)internalEntries[ind+1].lChild = intEntry.rChild;
        internalEntries[ind] = intEntry;
        break;
    }
   }
   for(int i = ind;i<blockHeader.numEntries;i++)
   {
    ...
    if (/* the current block was not the root */blockHeader.pblock != -1) {  // (check pblock in header)
        ...
        InternalEntry middle;
        middle.attrVal = internalEntries[MIDDLE_INDEX_INTERNAL].attrVal;
        middle.lChild = intBlockNum;
        middle.rChild = newRightBlk;
        int ret = insertIntoInternal(relId,attrName,blockHeader.pblock,middle);
        if(ret != SUCCESS)return ret;
...
int BPlusTree::splitInternal(int intBlockNum, InternalEntry internalEntries[]) {
    ...
    */
   for(int i = 0;i<MIDDLE_INDEX_INTERNAL;i++)leftBlk.setEntry(&internalEntries[i],i);
   for(int i = 0;i<MIDDLE_INDEX_INTERNAL;i++)rightBlk.setEntry(&internalEntries[MIDDLE_INDEX_INTERNAL+i+1],i);
    ...
    for (/* each child block of the new right block */int i = 0;i<MIDDLE_INDEX_INTERNAL;i++) {
    ...
    BlockBuffer block(internalEntries[1+MIDDLE_INDEX_INTERNAL].lChild);
    HeadInfo head;
    block.getHeader(&head);
    head.pblock = rightBlkNum;
    block.setHeader(&head);
...
int BPlusTree::createNewRoot(int relId, char attrName[ATTR_SIZE], Attribute attrVal, int lChild, int rChild) {
    ...
    left.getHeader(&leftheader);
    right.getHeader(&rightheader);
    leftheader.pblock = newRootBlkNum;
    rightheader.pblock = newRootBlkNum;
    left.setHeader(&leftheader);
    right.setHeader(&rightheader);
